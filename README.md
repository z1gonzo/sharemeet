...
> Current state: backend is build-green with Prisma 6, PostgreSQL on local port `5433`, email/password JWT auth, DTO validation, protected `GET /auth/me`, profile endpoints with follow counts, text posts CRUD/global feed/following feed with `PUBLIC`/`FOLLOWERS`/`PRIVATE` visibility, comments on public posts, and follow relationships (`POST/DELETE /users/:username/follow`, followers/following lists). Avatar/profile reporting is documented as future backlog. `npm run build`, `npm test` and `npm run test:e2e` pass. See `project_state.md`.
