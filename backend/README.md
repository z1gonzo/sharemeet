...
- `PostsModule` currently provides protected `POST /posts`, protected own-post `PATCH /posts/:id` and `DELETE /posts/:id`, `PUBLIC`/`FOLLOWERS`/`PRIVATE` post visibility, `commentsCount`, protected following feed `GET /posts/following?limit=20&offset=0`, public global feed `GET /posts?limit=20&offset=0` and public `GET /posts/:id`.
...
1. Add a "my posts" endpoint for private/own posts or start the basic frontend.
